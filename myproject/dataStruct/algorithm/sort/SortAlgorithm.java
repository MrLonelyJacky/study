...
public class SortAlgorithm {
    ...
    public static void insertSort(int[] arrays) {
        ...
        for (i = 1; i < arrays.length; i++) {
            ...
            if (arrays[i] < arrays[i - 1]) {
                int key = arrays[i];
                for (j = i - 1; j >= 0 && key < arrays[j]; j--) {
                    //当前元素后移
                    currentBackMove(arrays, j + 1, j);
                }
     ...
     * @param currentIndex
     */
    private static void currentBackMove(int[] arrays, int backIndex, int currentIndex) {
        arrays[backIndex] = arrays[currentIndex];
    }
