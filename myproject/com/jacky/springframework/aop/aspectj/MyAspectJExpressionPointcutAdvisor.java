package com.jacky.springframework.aop.aspectj;

import com.jacky.springframework.aop.MyPointcut;
import com.jacky.springframework.aop.MyPointcutAdvisor;
import lombok.Data;
import org.aopalliance.aop.Advice;

/**
 * AspectJExpressionPointcutAdvisor 实现了 PointcutAdvisor 接口，把切面 pointcut、拦截方法 advice 和具体的拦截表达式包装在一起。
 * 这样就可以在 xml 的配置中定义一个 pointcutAdvisor 切面拦截器了
 */
@Data
public class MyAspectJExpressionPointcutAdvisor implements MyPointcutAdvisor {
    // 切面
    private MyAspectJExpressionPointcut pointcut;
    // 具体的拦截方法
    private Advice advice;
    // 表达式
    private String expression;

    @Override
    public MyPointcut getPointcut() {
        if (null == pointcut) {
            pointcut = new MyAspectJExpressionPointcut(expression);
        }
        return pointcut;
    }

    @Override
    public Advice getAdvice() {
        return advice;
    }
}
